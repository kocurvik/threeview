...
experiments = ['4p(HC)', '5p3v', '5p3v + ENM', '5p3v + R + C', '5p3v + RR + CC', '5p3v + ENM + R + C', '5p3v + ENM + RR + CC', '4p3v(A) + R + C + ENM', '4p3v(M) + R + C + ENM','4p3v(M-D) + R + C + ENM',]
...
def get_rows(results, order, err_fun, runtime=True):
    ...
    for experiment in order:
        ...
        num_rows.append([p_avg, p_med, 100 * p_auc_5, 100 * p_auc_10, time_avg])
...
if __name__ == '__main__':
    ...
    generate_table(['pt', 'cambridge'] , 'superpoint', all_experiments=False, use_max_err=False)
