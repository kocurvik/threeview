...
def generate_graphs(dataset, results_type, all=True, basenames = None, exps=experiments, prefix='', **kwargs):
    ...
    for basename in basenames:
        ...
        with open(json_path, 'r') as f:
            ...
            draw_results_pose_auc_10(results, exps, iterations_list,
                                     f'{prefix}{dataset}_{basename}_{results_type}', err_fun=err_fun_main, **kwargs)
    ...
    if all:
        ...
        draw_results_pose_auc_10(all_results, exps, iterations_list, prefix + title, err_fun=err_fun_main, **kwargs)
        draw_results_pose_auc_10(all_results, exps, iterations_list, 'maxerr_' + prefix + title, err_fun=err_fun_max, **kwargs)
...
def generate_join_graphs(dataset, basenames = None, colors=None, linewidth=1.5):
    ...
    for t in ['3.0', '5.0', '10.0']:
        ...
        for experiment in tqdm(exps):
            ...
            plt.semilogx(xs, ys, label=f'{experiment} @ {t}px', marker='*', color=colors[experiment], linestyle=styles[t], linewidth=linewidth)
    ...
    plt.savefig(f'figs/{title}_pose.png', bbox_inches='tight', pad_inches=0.1)
    plt.legend()
    plt.savefig(f'figs/{title}_pose_legend.png', bbox_inches='tight', pad_inches=0.1)
...
if __name__ == '__main__':
    ...
    color_exps = [' ', ' ', '4p3v(A) + R + C + ENM', '4p3v(M)', '4p3v(M-D) + R + C', '4p3v(M-D) + R + C + ENM', '4p3v(M-D)',' ', '4p3v(M-D) + R']
